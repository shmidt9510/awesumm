MODULE user_case
  ...
  REAL (pr) :: a      ! Robin BC parameter
  REAL (pr) :: q_1    ! flux at top wall
  REAL (pr), DIMENSION(2) :: src      ! Coordinates of the heat source
...
!!$                Lu(iloc(1:nloc),ie) = du(ie,iloc(1:nloc),2)  !Neuman conditions
                ELSE IF( face(2) == -1  ) THEN
                    Lu(shift+iloc(1:nloc)) = a*u(shift+iloc(1:nloc)) + du(ie,iloc(1:nloc),2) ! Robin
                ELSE
...
!!$                Lu_diag(shift+iloc(1:nloc)) = du(iloc(1:nloc),2)  !Neuman conditions
                ELSE IF( face(2) == -1  ) THEN
                    Lu_diag(shift+iloc(1:nloc)) = a*1.0_pr + du(iloc(1:nloc),2) ! Robin
                ELSE
       ...
       DO face_type = 0, 3**dim - 1
          ...
          IF( ANY( face(1:dim) /= 0) ) THEN ! goes only through boundary points
             ...
             IF(nloc > 0 ) THEN 
                ...
                ELSE
                    rhs(shift+iloc(1:nloc)) = 0
                    WHERE (x(iloc(1:nloc), 1) >= src(1) .AND. x(iloc(1:nloc), 1) <= src(2))
  ...
  FUNCTION user_rhs (u_integrated,p)	 
    ...
    IF (IMEXswitch .GE. 0) THEN
       ...
       DO ie = n_var_temp, n_var_temp
          ...
          user_rhs(shift+1:shift+ng) = nu*SUM(d2u(ie,:,:),2)
       END DO
  ...
  FUNCTION user_Drhs_diag (meth)
    ...
    IF (IMEXswitch .GE. 0) THEN
       ...
       DO ie = n_var_temp, n_var_temp
          shift = ng*(ie-1)
          !CALL c_diff_fast(u_integrated(1:ng,ie), du, d2u, j_lev, nwlt, 1, 11, 1, 1, 1)
          user_Drhs_diag(shift+1:shift+ng) = nu*SUM(d2u,2)
       END DO
  ...
  call input_real ('a',a,'stop', &
       ' a: parameter for Robin BC')
  
  call input_real ('q_1',q_1,'stop', &
       ' q_1: flux at top wall')
  
  call input_real_vector ('source', src, 3, 'stop', &
       ' x-coords for surface source')

  ...
  FUNCTION user_rhs (u_integrated,p)	 
    ...
    IF (IMEXswitch .GE. 0) THEN
       ...
       DO ie = n_var_temp, n_var_temp
          ...
          user_rhs(shift+1:shift+ng) = nu*SUM(d2u(ie,:,:),2) - u_integrated(shift+1:shift+ng,ie)*du(ie,:,1)
       END DO
    ...
    IF (IMEXswitch .GE. 0) THEN
       ...
       DO ie = n_var_temp, n_var_temp
          ...
          user_Drhs(shift+1:shift+ng) = nu*SUM(d2u(ie,:,:),2) &
              - u(shift+1:shift+ng,ie)*du(ne+ie,:,1) - u_prev_timestep_loc(shift+1:shift+ng,ie)*du(ie,:,1)
       END DO
    ...
    IF (IMEXswitch .GE. 0) THEN
       ...
       DO ie = n_var_temp, n_var_temp
          ...
          user_Drhs_diag(shift+1:shift+ng) = nu*SUM(d2u,2) &
            - du(:,1)*u_prev_timestep(shift+1:shift+ng) - du_prev_timestep(ie,:,1)
       END DO

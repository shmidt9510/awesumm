...
!  USE debug_vars
  ...
  !
  INTEGER n_var_v     ! start of U in u array
  INTEGER n_var_u     ! start of V in u array
...
CONTAINS
  ...
  SUBROUTINE  user_setup_pde ( VERB )
    ...
    CALL register_var( 'U',integrated=.TRUE.,  adapt=(/.TRUE.,.TRUE./),   interpolate=(/.TRUE.,.TRUE./), &
        exact=(/.FALSE.,.FALSE./),   saved=.TRUE., req_restart=.TRUE., IC_MAP_NAME='U_old'  )
    CALL register_var( 'V', integrated=.TRUE., adapt=(/.TRUE.,.TRUE./), interpolate=(/.TRUE.,.TRUE./), &
        exact=(/.FALSE.,.FALSE./), saved=.TRUE., req_restart=.TRUE., IC_MAP_NAME='V_old' )
    CALL register_var( 'pressure ', integrated=.FALSE., adapt=(/.FALSE.,.FALSE./), interpolate=(/.FALSE.,.FALSE./), &
        exact=(/.FALSE.,.FALSE./), saved=.TRUE., req_restart=.FALSE., IC_MAP_NAME='pressure_old' )
    CALL setup_mapping()
...
!--********************************
  ...
  SUBROUTINE user_algebraic_BC (Lu, u, nlocal, ne_local, jlev, meth)
    ...
    DO ie = 1, ne_local
       ...
       DO face_type = 0, 3**dim - 1
          ...
          IF( face(1) /= 0 ) THEN ! goes only through boundary points
  ...
  FUNCTION user_rhs (u_integrated,p)
    ...
    IF (IMEXswitch .LE. 0) THEN
       DO ie = n_var_u, n_var_v
          shift = ng*(ie-1)
          IF( ie == n_var_u  ) THEN
             CALL c_diff_fast(u_integrated, du, d2u, j_lev, ng, meth, 10, ne, 1, ne)
             user_rhs(shift+1:shift+ng) = -u_integrated(:,ie)*du(ie,:,ie)
          ELSE
             CALL c_diff_fast(u_integrated**2, du, du_dummy, j_lev, ng, meth, 10, ne, 1, ne)
             user_rhs(shift+1:shift+ng) = -0.5_pr*du(ie,:,ie)
          END IF
       END DO
    END IF
    IF (IMEXswitch .GE. 0) THEN
       CALL c_diff_fast(u_integrated, du, d2u, j_lev, ng, meth, 01, ne, 1, ne)
       DO ie = n_var_u, n_var_v
          shift = ng*(ie-1)
          user_rhs(shift+1:shift+ng) = user_rhs(shift+1:shift+ng) + nu*SUM(d2u(ie,:,:),2)
       END DO
    END IF
  ...
  FUNCTION user_Drhs (u, u_prev_timestep_loc, meth)
    ...
    REAL (pr), DIMENSION (ng,ne) :: f
    INTEGER :: ie, shift
    REAL (pr), DIMENSION (ne,ng,dim) :: du, du_prev  ! 1st derivatives for u
    ...
    IF (IMEXswitch .LE. 0) THEN
       DO ie = n_var_u, n_var_v
          shift = ng*(ie-1)
          IF( ie == n_var_u  ) THEN
             CALL c_diff_fast(u, du, du_dummy, j_lev, ng, meth, 10, ne, 1, ne)
             CALL c_diff_fast(u_prev_timestep_loc, du_prev, du_dummy, j_lev, ng, meth, 10, ne, 1, ne)
             user_Drhs(shift+1:shift+ng) = - u(:,ie)*du_prev(ie,:,ie) - u_prev_timestep_loc(:,ie)*du(ie,:,ie)
          ELSE
             CALL c_diff_fast(u*u_prev_timestep_loc, du, du_dummy, j_lev, ng, meth, 10, ne, 1, ne)
             user_Drhs(shift+1:shift+ng) = - du(ie,:,ie)
          END IF
       END DO
    END IF
    IF (IMEXswitch .GE. 0) THEN
       CALL c_diff_fast(u, du, d2u, j_lev, ng, meth, 01, ne, 1, ne)
       DO ie = n_var_u, n_var_v
          shift = ng*(ie-1)
          user_Drhs(shift+1:shift+ng) = user_Drhs(shift+1:shift+ng) + nu*SUM(d2u(ie,:,:),2)
       END DO
    END IF
  ...
  FUNCTION user_Drhs_diag (meth)
    ...
    IF (IMEXswitch .LE. 0) THEN
       DO ie = n_var_u, n_var_v
          shift = ng*(ie-1)
          user_Drhs_diag(shift+1:shift+ng) = - du(:,ie)*u_prev_timestep(shift+1:shift+ng)
          IF( ie == n_var_u  ) THEN
    ...
    IF (IMEXswitch .GE. 0) THEN
       DO ie = n_var_u, n_var_v
          shift = ng*(ie-1)
          user_Drhs_diag(shift+1:shift+ng) = user_Drhs_diag(shift+1:shift+ng) + nu*SUM(d2u,2)
       END DO
    END IF
